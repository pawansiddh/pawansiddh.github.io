(async()=>{
  ...
  const loadScript=src=>new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error(`Failed to load ${src}`));document.head.appendChild(s)});
  const loadStyle=href=>new Promise((resolve,reject)=>{const l=document.createElement('link');l.rel='stylesheet';l.href=href;l.onload=resolve;l.onerror=()=>reject(new Error(`Failed to load ${href}`));document.head.appendChild(l)});
  try{
    ...
    await loadScript('finance-final-r3j.js?v=finance-final-r3k-20260828');
  }catch(e){
    ...
    try{await loadScript('finance-final-r3j.js?v=finance-final-r3k-20260828-fallback')}catch(_){ }
  }
})();
