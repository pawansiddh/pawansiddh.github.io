...
(() => {
  ...
  const edit=(kind,item,text='Edit')=>`<button type="button" class="mini-btn" onclick="trackerItemModal('${kind}','${item.id}')">${text}</button>`;
  const remove=(kind,item)=>`<button type="button" class="mini-btn danger-mini" aria-label="Delete ${h(label(kind,item))}" onclick="trackerDeleteItem('${kind}','${item.id}')">×</button>`;
  ...
  function mocksView(){
    ...
    return shell('mocks',`<div class="mock-lab"><section class="card mock-gauge"><div class="card-title"><h3>Current performance</h3><span>${rows.length} attempts</span></div><div class="mock-gauge-body"><canvas id="v42MockDonut"></canvas><div><strong>${latest}%</strong><small>${scores.length>1?`${delta>=0?'+':''}${delta}% from previous`:'Add a second attempt to compare'}</small></div></div></section><section class="card mock-trajectory"><div class="card-title"><h3>Score trajectory</h3><span>Attempt by attempt</span></div><canvas id="v42MockLine"></canvas></section><section class="card mock-weakness"><div class="card-title"><h3>Section diagnosis</h3><span>Low score first</span></div>${areaRows.length?areaRows.map(area=>`<div><span>${h(area.name)}</span><i><b style="width:${area.value}%"></b></i><strong>${area.value}%</strong></div>`).join(''):'<p>Add section scores such as “Networking: 8/10” to reveal strong and weak areas.</p>'}</section><section class="card mock-ledger"><div class="card-title"><h3>Attempt ledger</h3><span>${rows.length}</span></div>${rows.length?rows.slice().reverse().map(item=>`<article><time>${h(item.date||'No date')}</time><span><strong>${h(item.name)}</strong><small>${h(meta('mocks',item))}</small></span><b>${score(item)}%</b>${edit('mocks',item)}${remove('mocks',item)}</article>`).join(''):empty('mocks')}</section></div>`);
  }
  ...
  function drawMockCharts(rows){
    if(!window.Chart)return;resetChart('mockDonut');resetChart('mockLine');const theme=chartTheme(),scores=rows.map(score),latest=scores.at(-1)||0,common={responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}};const donut=document.querySelector('#v42MockDonut');if(donut)charts.mockDonut=new Chart(donut,{type:'doughnut',data:{datasets:[{data:[latest,100-latest],backgroundColor:['#d65778',theme.line],borderWidth:0}]},options:{...common,cutout:'74%',plugins:{legend:{display:false},tooltip:{enabled:false}}}});const line=document.querySelector('#v42MockLine');if(line)charts.mockLine=new Chart(line,{type:'line',data:{labels:rows.map((item,index)=>item.date||`#${index+1}`),datasets:[{data:scores,borderColor:'#d65778',backgroundColor:'#d6577822',fill:true,tension:.32,pointRadius:3}]},options:{...common,scales:{y:{beginAtZero:true,max:100,ticks:{color:theme.muted},grid:{color:theme.line}},x:{ticks:{color:theme.muted},grid:{display:false}}}}});
  }
