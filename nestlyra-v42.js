...
(() => {
  ...
  function certificationsView(){
    ...
    const stageMarkup=stages.map((stage,index)=>{const count=rows.filter(item=>item.status===stage).length;return `<span class="${count?'has-items':''}"><i aria-hidden="true">${index+1}</i><div><strong>${stage}</strong><small>${count} ${count===1?'credential':'credentials'}</small></div></span>`}).join('');
    return shell('certifications',`<section class="cert-path card"><div class="cert-stage-line" aria-label="Certification pathway stages">${stageMarkup}</div><div class="cert-cards">${rows.map(item=>{const pct=progress('certifications',item),days=dateDelta(item.examDate);return `<article><div class="v42-ring" style="--p:${pct}"><strong>${pct}%</strong></div><div><small>${h(item.provider||'Credential')}</small><h3>${h(label('certifications',item))}</h3><p>${h(item.nextStep||'Add the next evidence milestone')}</p><span>${days===null?'No exam date':days<0?'Exam date passed':`${days} days to exam`} · ${h(item.status||'Planning')}</span></div><footer>${edit('certifications',item,'Open pathway')}${remove('certifications',item)}</footer></article>`}).join('')}</div></section>`);
  }
  ...
  function chartTheme(){const css=getComputedStyle(document.body);return {accent:css.getPropertyValue('--chart-1').trim()||css.getPropertyValue('--accent').trim()||'#315f46',accent2:css.getPropertyValue('--chart-2').trim()||css.getPropertyValue('--accent2').trim()||'#8ba26b',muted:css.getPropertyValue('--muted').trim()||'#66708b',line:css.getPropertyValue('--line').trim()||'#dfe4ef'}}
  ...
  function drawDashboard(){
    ...
    const bars=document.querySelector('#v42ModuleBars');if(bars)charts.modules=new Chart(bars,{type:'bar',data:{labels:usable.map(item=>item.label),datasets:[{data:usable.map(item=>clamp(item.value)),backgroundColor:usable.map((_,index)=>index%2?theme.accent:theme.accent2),borderRadius:6,maxBarThickness:15}]},options:{...common,indexAxis:'y',scales:{x:{beginAtZero:true,max:100,ticks:{color:theme.muted,font:{size:8}},grid:{color:theme.line}},y:{ticks:{color:theme.muted,font:{size:8}},grid:{display:false}}},plugins:{legend:{display:false}}}});
  ...
  function drawPracticeChart(rows){
    if(!window.Chart)return;resetChart('practice');const theme=chartTheme(),canvas=document.querySelector('#v42PracticeLine');if(canvas)charts.practice=new Chart(canvas,{type:'line',data:{labels:rows.map((item,index)=>item.date||`#${index+1}`),datasets:[{label:'Accuracy %',data:rows.map(score),borderColor:theme.accent2,backgroundColor:`${theme.accent2}22`,fill:true,tension:0,pointRadius:3},{label:'Minutes',data:rows.map(item=>Number(item.duration||0)),borderColor:theme.accent,pointRadius:2,borderDash:[4,4]}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,ticks:{color:theme.muted},grid:{color:theme.line}},x:{ticks:{color:theme.muted},grid:{display:false}}},plugins:{legend:{display:true,position:'bottom',labels:{color:theme.muted,boxWidth:8}}}}});
  }
