...
(() => {
  ...
  const MODULE_HELP_SECTIONS = [
    ...
    ['resources','C6','Resource Library','Save links and keep large PDFs in your own Google Drive.',`<ol><li>Add a title and choose Book, Course, Video, Article, Tool, Website, Google Drive PDF or Other.</li><li>For a PDF, choose <strong>Add Drive PDF</strong>. Sign in with Google, approve the limited <code>drive.file</code> permission, and select a PDF up to 50 MB.</li><li>Nestlyra creates <code>Nestlyra/documents</code> in your Drive, uploads the PDF there and stores only its file ID, name, size and link in the tracker.</li><li>Use Saved, In use and Completed to control the reading queue. Turning the module off never deletes the Drive file.</li></ol>`],
