...
assert.match(html,/One secure Nestlyra account/);
assert.match(html,/src="groups\.js\?v=44"/);
assert.match(html,/src="nestlyra-v39\.js\?v=44"/);
assert.match(html,/src="nestlyra-v42\.js\?v=44"/);
...
assert.match(schema,/membership\.group_id=chosen\.group_id/,'Invite redemption must qualify group_id to avoid PostgreSQL ambiguity');
assert.match(schema,/#variable_conflict use_column/,'Invite redemption must explicitly resolve returned group_id against table columns');
...
assert.match(worker,/nestlyra-focus-v44-export-walkthrough/);
assert.match(worker,/skipWaiting/);
assert.match(worker,/clients\.claim/);
assert.match(worker,/\.\/groups\.js/);
assert.match(worker,/\.\/supabase-groups-migration\.sql/);
assert.match(worker,/\.\/supabase-v42-hotfix\.sql/);
