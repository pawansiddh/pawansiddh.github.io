...
window.copyMessagingDatabaseFix = async () => {
  try {
    const response = await fetch('./supabase-tracker-v31-hotfix.sql', {cache:'no-store'});
...
window.familyOpenMessaging = () => {
  if (!isCloudParentSession?.()) return modal(`<div class="cloud-parent-setup"><span class="eyebrow">ENABLE CLOUD MESSAGING</span><h2>Connect a cloud Parent account</h2><p>Your local Parent ID remains private to this browser. Cross-device messages need a free Supabase-backed Parent account so the linked learner can securely identify you.</p><ol><li>Return to Parent login.</li><li>Use a Parent email and password, or Continue with Google as Parent.</li><li>Generate a new Family code and ask the learner to redeem it.</li></ol><div class="modal-actions"><button class="btn ghost">Not now</button><button type="button" class="btn primary" onclick="familyStartCloudParentSetup()">Go to cloud Parent login</button></div></div>`);
...
window.familyStartCloudParentSetup = () => {
  closeModal();
  sessionStorage.setItem('studyTracker.openParentLogin','1');
  familyParentLogout();
};
...
async function familyMessagingBoot(parentMode = false) {
  ...
  if (error) {
    ...
    root.innerHTML = `<div class="family-chat-empty"><span>!</span><h3>${ambiguous?'One-time Nestlyra Focus database update required':'Messaging is not ready'}</h3><p>${ambiguous?'The installed Family functions are an older version. Copy the v31 update below and run it once in the Supabase SQL Editor. It also enforces one active linking code and read-only Parent Observer access.':familyChatEscape(error.message)}</p>${ambiguous?'<div class="family-chat-fix-actions"><button type="button" class="btn primary" onclick="copyMessagingDatabaseFix()">Copy v31 SQL update</button><a class="btn ghost" href="supabase-tracker-v31-hotfix.sql" download>Download SQL update</a></div>':'<small>Run the current Supabase Family schema, then refresh this page.</small>'}</div>`;
    return;
  }
...
window.enhanceSettingsNavigation = () => {
  ...
  const categoryFor = title => {
    if (/Navigation & modules/.test(title)) return 'modules';
