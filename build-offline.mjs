...
const verifiedOutput='TULSHII-Focus-Offline-Verified-v6.html';
...
fs.writeFileSync(path.join(root,verifiedOutput),html);
console.log(`Built tulshii-focus-offline.html and ${verifiedOutput} (${(Buffer.byteLength(html)/1024/1024).toFixed(2)} MiB each)`);
...
function buildScripts(){
  ...
  const transformed=scriptFiles.map(file=>{
    ...
    if(file==='app.js'){
      source=source
        .replace("const KEY='studyTracker.v30',CLOUD_ACCOUNT_KEY='studyTracker.cloudAccount',APPEARANCE_KEY='studyTracker.appearance.v1',NOTICE_KEY='studyTracker.notice',LAST_VIEW_PREFIX='studyTracker.lastView.v1';","const KEY='tulshiiFocusOffline.workspace.v6',CLOUD_ACCOUNT_KEY='tulshiiFocusOffline.cloudAccount.v6',APPEARANCE_KEY='tulshiiFocusOffline.appearance.v6',NOTICE_KEY='tulshiiFocusOffline.notice.v6',LAST_VIEW_PREFIX='tulshiiFocusOffline.lastView.v6';")
    ...
    if(file==='family.js'){
      source=source
        .replace("const FAMILY_KEY='studyTracker.family.v1';","const FAMILY_KEY='tulshiiFocusOffline.preferences.v6';")
        .replace('return `studyTracker.briefing.${cloudFamilyUser?.id||state?.profile?.familyUserId||\'local\'}`','return `tulshiiFocusOffline.briefing.v6.${cloudFamilyUser?.id||state?.profile?.familyUserId||\'local\'}`')
    ...
    if(file==='focus-v48.js'){
      source=source
        .replace("const SIDEBAR_KEY='pavenro.focus.sidebar.collapsed.v1';","const SIDEBAR_KEY='tulshiiFocusOffline.sidebar.v6';")
    ...
    if(file==='nestlyra-v37.js'||file==='nestlyra-v42.js'){
      source=source
        .replaceAll("'studyTracker.appearance.v1'","'tulshiiFocusOffline.appearance.v6'")
