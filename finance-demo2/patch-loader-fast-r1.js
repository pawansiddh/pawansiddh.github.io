(async()=>{
  ...
  const report={version:'finance-demo2-consolidated-r3-auditfix',startedAt:new Date().toISOString(),loaded:[],failed:[],skipped:['patch6 (known malformed and never part of effective Demo 2 runtime)'],unpacked:[],policies:[],timings:{}};
  ...
  const slowIntervals=new Set(['phase1-status','interaction-audit','daily-briefing-r2','phase2-r1','brand-sidebar-r1','ui-controls-r1']);
  const blockBroadObservers=new Set(['finance-debt-lab-r1','finance-theme-studio-r1']);
  const execWithPolicy=(name,text)=>{
    if(!text)return false;
    const NativeMO=window.MutationObserver, nativeInterval=window.setInterval, nativeAdd=EventTarget.prototype.addEventListener;
    let moChanged=false,intervalChanged=false,addChanged=false;
    try{
      if(blockBroadObservers.has(name)&&NativeMO){
        window.MutationObserver=class PavenroScopedMutationObserver extends NativeMO{
          observe(target,options){
            if((target===document.body||target===document.documentElement)&&options?.subtree){report.policies.push(`${name}:blocked-broad-mutation-observer`);return}
            return super.observe(target,options);
          }
        };
        moChanged=true;
      }
      if(slowIntervals.has(name)){
        window.setInterval=(fn,ms,...args)=>nativeInterval(fn,Math.max(Number(ms)||0,5000),...args);
        intervalChanged=true;report.policies.push(`${name}:minimum-5s-safety-poll`);
      }
      if(name==='interaction-audit'){
        EventTarget.prototype.addEventListener=function(type,listener,options){
          if(this===document&&type==='click'&&listener?.name==='intercept'){
            const wrapped=function(e){const b=e.target?.closest?.('button');if(b&&(b.matches('.pv-p1-action,.pv-c2-action,.pv-pl-action,.pv-r-action')||b.closest('.pv-p1-modal,.pv-c2-modal,.pv-pl-modal,.pv-r-modal,.pv-pl-panel,.pv-r-panel')))return;return listener.call(this,e)};
      ...
      if(moChanged)window.MutationObserver=NativeMO;
      if(intervalChanged)window.setInterval=nativeInterval;
      if(addChanged)EventTarget.prototype.addEventListener=nativeAdd;
    }
  };
  const execAsset=async(name,text)=>execWithPolicy(name,await unwrapRuntime(name,text));
  ...
  const V='finance-demo2-auditfix-r3-20260829';
  const assetDefs=[
    ...Array.from({length:4},(_,i)=>[`patch4-${i}`,`patch/${i}.txt?v=${V}`,true]),['patch-v5',`patch-v5.js?v=${V}`,true],['baseline-js',`finance-baseline-v3.js?v=${V}`,true],['baseline-css',`finance-baseline-v3.css?v=${V}`,true],['search-r3c',`finance-search-controller-r3.js?v=${V}`,true],['sidebar-css',`finance-sidebar-clean-r1.css?v=${V}`,true],['debt-r1',`finance-debt-lab-r1.js?v=${V}`,false],['theme-r1',`finance-theme-studio-r1.js?v=${V}`,false],['bell-r1',`finance-bell-contrast-r1.js?v=${V}`,false],['state-bridge',`finance-state-bridge-r1.js?v=${V}`,true],['phase1-r3',`finance-phase1-r3.js?v=${V}`,false],['phase1-core2',`finance-phase1-core2-r1.js?v=${V}`,false],['phase1-planning',`finance-phase1-planning-r1.js?v=${V}`,false],['phase1-records',`finance-phase1-records-r1.js?v=${V}`,false],['phase1-status',`finance-phase1-status-fix-r1.js?v=${V}`,false],['interaction-audit',`finance-interaction-audit-r1.js?v=${V}`,false],['daily-briefing-r2',`finance-daily-briefing-r2.js?v=${V}`,false],['calendar-studio-r1',`finance-calendar-studio-r1.js?v=${V}`,false],['coedit-audit-r1',`finance-coedit-audit-r1.js?v=${V}`,false],['offline-sync-r1',`finance-offline-sync-r1.js?v=${V}`,false],['phase2-r1',`finance-phase2-r1.js?v=${V}`,false],['brand-sidebar-r1',`finance-brand-sidebar-r1.js?v=${V}`,false],['ui-controls-r1',`finance-ui-controls-r1.js?v=${V}`,false]
  ];
  ...
  try{
    ...
    await execAsset('finance-baseline-v3',A['baseline-js']);style('finance-baseline-v3-css',A['baseline-css']);
    await execAsset('finance-search-r3c',A['search-r3c']);style('finance-sidebar-clean-r1-css',A['sidebar-css']);
    await execAsset('finance-debt-lab-r1',A['debt-r1']);await execAsset('finance-theme-studio-r1',A['theme-r1']);await execAsset('finance-bell-contrast-r1',A['bell-r1']);
