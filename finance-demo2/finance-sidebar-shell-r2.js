(()=>{
  ...
  const style=document.createElement('style');style.id='pv-sidebar-shell-r2-style';style.textContent=`
    ...
    .sidebar .brand>hr,.sidebar .brand>.divider,.sidebar .brand>.separator,.sidebar .brand>.pv-sep,.sidebar .brand>small{display:none!important}
    .sidebar .brand #pvPawanroWordmark{width:132px!important;max-width:calc(100% - 4px)!important;height:34px!important;margin:0!important;object-fit:contain!important;object-position:left center!important;flex:0 1 auto!important}
    ...
    body.pv-rail .sidebar .brand #pvPawanroWordmark{display:none!important}
  ...
  function allowedBrandChild(el){return el?.id==='pvPawanroWordmark'||el?.classList?.contains('pv-pawanro-mini')}
  ...
  function utility(s,name,icon){
    ...
    let label=q('.pv-util-label',b);if(!label){
      const old=norm(b.textContent);b.textContent='';const i=document.createElement('span');i.className='pv-util-icon';i.textContent=icon;i.setAttribute('aria-hidden','true');label=document.createElement('span');label.className='pv-util-label';b.append(i,label);
    }
  ...
  function refresh(){
    ...
    cleanBrand(s);cleanDuplicates(s);hideObsoleteLabels(s);
