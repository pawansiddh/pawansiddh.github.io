...
window.familyScheduleBriefing=()=>{if(!familyShouldShowBriefing())return;setTimeout(()=>{const dialog=document.querySelector('#modal');if(dialog?.open)return;welcomeBriefing();familyConfigureBriefing();familyOnBriefing()},Math.max(300,Number(familyData.settings.briefingDelay||.3)*1000))};
